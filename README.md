...
Сгенерируйте файлы командой `python3 example.py`. Генератор содержит:
...
RETURN   непоср. адресация: 0       EXIT
```
...
RETURN    непоср. адресация: 2        EXIT
```

Значимые пути в сети `1.rn`:
